...
# Definition of function for Filters
def get_filters():
    """
    ...
    """
    print('Hello! Let us explore the US bikeshare data!')
    ...
    while True:
        ...
        month = input("\n Which month would you like to analyse? (January, February, March, April, May, June)? Type 'None' for no month filter\n").title()
        if month in months:
            break
        else:
    ...
    while True:
        ...
        day = input("\n Which day of the week would you like to analyse? (Monday, Tuesday, Wednesday, Thursday, Friday, Saturday, Sunday)? Type 'None' for no day filter \n").title()         
        if day in days:
            break
        else:
...
def load_data(city, month, day):
    ...
    Arguments:
        ...
        (str) month - name of the month to filter, or "None" to apply no filter
        (str) day - name of the day of week to filter, or "None" to apply no filter
    Returns:
...
def station_stats(df):
    ...
    print('\nCalculating the most popular stations (from/to)...\n')
    start_time = time.time()

    # display most commonly used start station
    pop_start_station= df['Start Station'].mode()[0]
    print("The most commonly used starting point is {}".format(pop_start_station))


    # display most commonly used end station
    pop_end_station= df['End Station'].mode()[0]
    print("The most commonly used ending point is {}".format(pop_end_station))

    # display most frequent combination of start station and end station trip
    df['combination']=df['Start Station']+" "+"to"+" "+ df['End Station']
    pop_com= df['combination'].mode()[0]
    print("The most frequent total trip is {} ".format(pop_com))
